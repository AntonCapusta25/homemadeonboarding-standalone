...
export interface ChefWithStats extends ChefProfile {
  ...
  verification_kitchen_verified?: boolean;
  ...
  const fetchChefs = useCallback(async () => {
    ...
    try {
        ...
        if (!verificationResult.error && verificationResult.data) {
          verificationDataMap = verificationResult.data.reduce((acc, v) => {
            acc[v.chef_profile_id] = {
              ...
              kitchen_verified: !!v.kitchen_verified_at,
      ...
      let mergedData: ChefWithStats[] = (profilesData || []).map(chef => ({
        ...
        verification_kitchen_verified: verificationDataMap[chef.id]?.kitchen_verified || false,
